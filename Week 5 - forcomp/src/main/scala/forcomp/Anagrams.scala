...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences](Nil)) {
      case ((character, count), acc) => {
        acc ++ (
          for {
            comb <- acc
            n <- 1 to count
          }
          yield (character, n) :: comb
        )
      }
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    (y foldLeft SortedMap[Char, Int]() ++ x) { case (map, (character, count))
    => {
      val newCount = map(character) - count
      if (newCount != 0) map updated (character, newCount)
      else map - character
    }}.toList
