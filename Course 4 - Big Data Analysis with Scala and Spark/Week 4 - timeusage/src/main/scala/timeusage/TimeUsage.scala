...
//    dataFrame.show()
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def getNeeds(names: List[String], identifiers: List[String]): List[String] =
      names.filter(column => identifiers.exists(column.contains(_)))

    val primaryNeeds = getNeeds(columnNames, List("t01", "t03", "t11", "t1801", "t1803"))
    val workingActivities = getNeeds(columnNames, List("t05", "t1805"))
    val otherActivities = getNeeds(
      columnNames.filter(column => !primaryNeeds.contains(column) && !workingActivities.contains(column)),
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column =
        when(col("teage").between(15, 22), "young")
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy(col("working"), col("sex"), col("age"))
      ...
      .withColumn("primaryNeeds", round(col("avg(primaryNeeds)"), 1))
      .withColumn("work", round(col("avg(work)"), 1))
      .withColumn("other", round(col("avg(other)"), 1))
      .drop("avg(primaryNeeds)", "avg(work)", "avg(other)")
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"""
      |SELECT working, sex, age, ROUND(AVG(primaryNeeds), 1) as primaryNeeds, ROUND(AVG(work), 1) as work, ROUND(AVG
      |(other), 1) as other
      |FROM $viewName
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def roundTenths(num: Double) = (num * 10).round / 10D
    ...
    summed.groupByKey(row => (row.working, row.sex, row.age))
      ...
      case ((working, sex, age), primaryNeeds, work, other) => TimeUsageRow(working, sex, age, roundTenths
      (primaryNeeds), roundTenths(work), roundTenths(other))
