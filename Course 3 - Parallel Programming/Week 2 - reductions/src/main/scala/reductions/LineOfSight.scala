...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    var i = from
    var maxAngle = if (i == 0) i else input(i)/ i
    while (i < until) {
      maxAngle = if (i == 0) i else max(maxAngle, input(i) / i)
      i = i + 1
    }
    maxAngle
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    var i = from
    while(i < until) {
      val currentAngle = if (i == 0) i else input(i) / i
      output(i) = if (i == 0) i else max(output(i-1), currentAngle)
      i = i + 1
    }
  }
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float],
    threshold: Int): Unit = {
    val reduction = upsweep(input, 0, input.length, threshold)
    downsweep(input, output, 0, reduction)
  }
}
