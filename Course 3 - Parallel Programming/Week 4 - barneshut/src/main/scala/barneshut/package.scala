...
package object barneshut {
  ...
  case class Fork(
    ...
    val size: Float = math.max(nw.size, math.max(ne.size, math.max(sw.size, se.size)))
    val mass: Float = nw.mass + ne.mass + sw.mass + se.mass
    val massX: Float = if (mass == 0f) centerX else
      (nw.massX * nw.mass + ne.massX * ne.mass + sw.massX * sw.mass + se.massX * se.mass) / mass
    val massY: Float = if (mass == 0f) centerY else
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        (bodies :+ b).foldLeft(Fork(nw, ne, sw, se))((fork: Fork, body: Body) => fork.insert(body))
      }
      else {
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Leaf(_, _, _, bodies) =>
          ...
          bodies.foreach((body: Body) => addForce(body.mass, body.x, body.y))
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def clamp(num: Float, min: Float): Int = {
      ((num - min) / sectorSize).toInt max 0 min sectorPrecision - 1
    }

    def +=(b: Body): SectorMatrix = {
      val row = clamp(b.y, boundaries.minY)
      val col = clamp(b.x, boundaries.minX)
